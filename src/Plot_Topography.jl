...
function PlotTopography(
    channelList::Vector{String},
    channelValues::Vector{T};
    gridSize = 1000,
) where {T<:Number}

    # Basic checking
    length(channelList) == length(channelValues) ||
        error("The lengths of channelList($(length(channelList))) and 
    ...
    standardChannelLabel, standardCoordinateX, standardCoordinateY =
        standardChannels.label, standardChannels.x, standardChannels.y
