...
const MY_LATEX_CMD = ["latexmk", "-pdfxe", "-xelatex", "-8bit", "-shell-escape", "-synctex=1", "-interaction=nonstopmode", "-file-line-error"]
...
function __init__()
    ...
    MintedDefaultDict = Dict(property => getproperty(MintedDefault, property) for property ∈ propertynames(MintedDefault))
    ...
    Weave.register_format!("minted2pdf", 
        Weave.LaTeX2PDF(
            primaryformat = Weave.LaTeXMinted(
            codestart   = "\\begin{compactminted}[texcomments = true, mathescape, bgcolor = GhostWhite, frame = none]{julia}",
            codeend     = "\\end{compactminted}",
            outputstart = "\\begin{compactminted}[texcomments = true, mathescape, bgcolor = GhostWhite, frame = leftline]{julia}",
...
Weave.render_chunk(docformat::Weave.LaTeXMinted, chunk::Weave.DocChunk) = join((Weave.render_inline(c) for c in chunk.content))
Weave.render_doc(docformat::Weave.LaTeXMinted, body, doc) = Mustache.render( mt"{{{ :body }}}"; body = body)
...
function findRelevantFiles(filepath)
    ...
    RemveExtensions  = ["aux", "xdv", "toc", "synctex.gz", "out", "log", "fls", "fdb_latexmk", "blg", "bbl"]
...
import DataFrames: _show, AbstractDataFrame, _check_consistency, _names, latex_escape, batch_compacttype, SHOW_TABULAR_TYPES, ourshow
